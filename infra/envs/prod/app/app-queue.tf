    ...
    template {
      ...
      spec {
        ...
        container {
          ...
          port { container_port = 8082 } # Queue 워커 포트
          ...
          startup_probe { tcp_socket { port = 8082 }, initial_delay_seconds = 10, period_seconds = 5, failure_threshold = 30 }
          readiness_probe { http_get { path = "/actuator/health", port = 8082 }, initial_delay_seconds = 30, period_seconds = 10 }
          liveness_probe { http_get { path = "/actuator/health/liveness", port = 8082 }, initial_delay_seconds = 60, period_seconds = 15 }
        }
